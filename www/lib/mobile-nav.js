...
.provider('$navigate', function() {
  ...
  this.$get = ['$rootScope', '$location', '$route', function($rootScope, $location, $route) {
    ...
    function navigate(destination, source, isBack, operator) {
      $rootScope.$broadcast('$pageTransitionStart', destination, source, isBack, operator);
     ...
     */
    nav.go = function go(path, transition, isReverse, operator) {
      ...
      nav.onRouteSuccess = function($event, next, last) {
        ...
        navigate(nav.next, nav.current, false, operator);
      };
    };
    ...
    nav.back = function(operator) {
      if (navHistory.length > 0) {
        ...
        nav.onRouteSuccess = function() {
          navHistory.pop();
          nav.next = previous;
          navigate(nav.next, nav.current, true, operator);
        };
        return true;
      }
      return false;
...
function($rootScope, $compile, $controller, $route, $change, $q) {
  ...
  function link(scope, viewElement, attrs) {    
    ...
    scope.$on('$pageTransitionSuccess', function ($event, dest, source) {
    })

    var currentTrans;
    scope.$on('$pageTransitionStart', function ($event, dest, source, reverse, operator) {
      function changePage() {
        var current = $route.current ? $route.current.$$route : {};
        var transition = reverse ? source.transition() : dest.transition();

        insertPage(dest);
        if(checkActionOverflow) checkActionOverflow();
        ...
        function doTransition() {
          ...
          promise.then(function() {
            if (source) {
              ...
              // alert('removed: ' + source.controller.constructor.toString())
              source = undefined;
            }
          });
